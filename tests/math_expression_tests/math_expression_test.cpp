...
const suite<"formula"> _ = [] {
    ...
    "polish_notation_throws"_test = [] {
        ...
        // must be nothrow
        expect(nothrow([]() { math_expression<T>{"x y z: 0."}; }));
        expect(nothrow([]() { math_expression<T>{"x y z: 0. * x / y"}; }));
        expect(nothrow([]() { math_expression<T>{"x y z: .5"}; }));
