    ...
    for (const size_t element_order : std::ranges::iota_view(1zu, 6zu)) {
        ...
        test("copy" + suffix) = [&element] {
            ...
            for (const T node : Sample_Nodes) {
                for (const size_t i : element->nodes())
                    expect(eq(copy->N(i, node), element->N(i, node))) << "Unexpected value of function " + std::to_string(i) +
        ...
        for (const size_t quadrature_order : std::ranges::iota_view{ element_order, 6zu }) {
            ...
            test("weights" + quadrature_suffix) = [&integrated_element] {
                const T element_length =
                    integrated_element.element().boundary(side_1d::RIGHT) - integrated_element.element().boundary(side_1d::LEFT);
                const auto qnodes = integrated_element.qnodes();
                const T weights_sum =
                    std::accumulate(qnodes.begin(), qnodes.end(), T{ 0 }, [&integrated_element](const T sum, const size_t qnode) {
            ...
            test("integrated_copy" + quadrature_suffix) = [&integrated_element] {
                ...
                expect(eq(copy.qnodes_count(), integrated_element.qnodes_count()))
                    << "Unexpected qnodes count in copied element.";
                ...
                for (const size_t i : integrated_element.nodes())
                    for (const size_t q : integrated_element.qnodes()) {
                        expect(eq(copy.qN(i, q), integrated_element.qN(i, q))) << "Unexpected value of function " +
                                                                                      std::to_string(i) + " at qnode " +
                                                                                      std::to_string(q) + " in copied element.";
                        expect(eq(copy.qNxi(i, q), integrated_element.qNxi(i, q)))
                            << "Unexpected derivative of function " + std::to_string(i) + " at qnode " + std::to_string(q) +
                                   " in copied element.";
                    };
            };
        }
