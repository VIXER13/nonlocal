...
const suite<"quadrature_2d"> _ = [] {
    for (const auto type : { geometry_t::Rectangle, geometry_t::Triangle })
        for (const size_t order : std::ranges::iota_view{ 1zu, 7zu }) {
            if (type == geometry_t::Rectangle && order > 5)
                continue;
            ...
            test("nodes_count" + suffix) = [&quadrature, type, order] {
                ...
                expect(eq(quadrature->nodes_count(), type == geometry_t::Triangle ? Nodes_Count[order - 1] : order * order))
                    << "Unexpected nodes count.";
            };
            ...
            test("weights_sum" + suffix) = [&quadrature] {
                ...
                expect(approx(weights_sum, dynamic_cast<const geometry_2d_base<T>&>(*quadrature).area(), Epsilon))
                    << "Unexpected weights sum.";
            };

            test("copy" + suffix) = [&quadrature] {
                const auto copied_quadrature = quadrature->copy();
                expect(eq(copied_quadrature->nodes_count(), quadrature->nodes_count()))
                    << "Unexpected nodes count in copied quadrature.";
                for (const size_t i : quadrature->nodes()) {
                    ...
                    expect(eq(copied_quadrature->node(i)[0], quadrature->node(i)[0]))
                        << "Unexpected node coordinate 0 in copied quadrature.";
                    expect(eq(copied_quadrature->node(i)[1], quadrature->node(i)[1]))
                        << "Unexpected node coordinate 1 in copied quadrature.";
                }
            };
        }
};
