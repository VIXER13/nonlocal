...
const suite<"anisotropic_lame_solid_ring"> _ = [] {
    ...
    const T A = Delta * (std::pow(Inner_Radius, k + 1) * Inner_Pressure - std::pow(Outer_Radius, k + 1) * Outer_Pressure) /
                (Er * (k + nu_fr));
    const T B =
        Delta * std::pow(Inner_Radius * Outer_Radius, k) *
        (Inner_Radius * std::pow(Outer_Radius, k) * Inner_Pressure - Outer_Radius * std::pow(Inner_Radius, k) * Outer_Pressure) /
        (Er * (k - nu_fr));
