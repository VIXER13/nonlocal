...
const suite<"saint_venant"> _ = [] {
    ...
    const raw_mechanical_parameters<T> parameters = {
        { "DEFAULT",
          { .physical = { .elastic = isotropic_elastic_parameters<T>{ .young_modulus = 1., .poissons_ratio = 0.3 } } } }
    };
    ...
    boundaries_conditions["Horizontal"] = { std::make_unique<pressure_2d<T>>(T{ 0 }),
                                            std::make_unique<displacement_2d<T>>(T{ 0 }) };
    boundaries_conditions["Left"] = { std::make_unique<pressure_2d<T>>(
                                          [](const std::array<T, 2>& point) { return -4 * std::abs(point[Y]); }),
                                      std::make_unique<pressure_2d<T>>(T{ 0 }) };
    boundaries_conditions["Right"] = { std::make_unique<pressure_2d<T>>(
                                           [](const std::array<T, 2>& point) { return 4 * std::abs(point[Y]); }),
                                       std::make_unique<pressure_2d<T>>(T{ 0 }) };
