...
const suite<"isotropic_lame_composite_ring"> _ = [] {
    ...
    const raw_mechanical_parameters<T> parameters = {
        ...
        { "Outer_Material",
          { .physical = { .elastic = isotropic_elastic_parameters<T>{ .young_modulus = 150., .poissons_ratio = 0.2 } } } }
    };
    ...
    boundaries_conditions["Inner"] = { std::make_unique<pressure_2d<T>>([](const std::array<T, 2>& point) {
                                           return Inner_Pressure * point[X] / std::hypot(point[X], point[Y]);
                                       }),
                                       std::make_unique<pressure_2d<T>>([](const std::array<T, 2>& point) {
                                           return Inner_Pressure * point[Y] / std::hypot(point[X], point[Y]);
                                       }) };
    ...
    boundaries_conditions["Outer"] = { std::make_unique<pressure_2d<T>>([](const std::array<T, 2>& point) {
                                           return -Outer_Pressure * point[X] / std::hypot(point[X], point[Y]);
                                       }),
                                       std::make_unique<pressure_2d<T>>([](const std::array<T, 2>& point) {
                                           return -Outer_Pressure * point[Y] / std::hypot(point[X], point[Y]);
                                       }) };
    ...
    "strain"_test = [&mesh, &solution] {
        static constexpr auto Expected_Strain = [](const std::array<T, 2>& point) {
            ...
            const auto Inner_Solution = [diff, mul]() {
                return std::array{ (T{ 1 } + T{ 2.5284 } * diff) / T{ 47303 }, (T{ 1 } - T{ 2.5284 } * diff) / T{ 47303 },
                                   mul / T{ 9354.31 } };
            };
            const auto Outer_Solution = [diff, mul]() {
                return std::array{ (T{ -1 } + T{ 9.7572 } * diff) / T{ 140715 }, (T{ -1 } - T{ 9.7572 } * diff) / T{ 140715 },
                                   mul / T{ 7210.83 } };
            };
            if (is_contact(r2))
    ...
    "stress"_test = [&mesh, &solution] {
        static constexpr auto Expected_Stress = [](const std::array<T, 2>& point) {
            ...
            using namespace metamath::operators;
            const auto Inner_Solution = [diff, mul]() {
                return std::array{ (T{ 1 } + T{ 1.5170 } * diff) / T{ 101.364 }, (T{ 1 } - T{ 1.5170 } * diff) / T{ 101.364 },
                                   mul / T{ 33.4082 } };
            };
            const auto Outer_Solution = [diff, mul]() {
                return std::array{ -(T{ 1 } - T{ 6.5048 } * diff) / T{ 750.481 }, -(T{ 1 } + T{ 6.5048 } * diff) / T{ 750.481 },
                                   mul / T{ 57.6866 } };
            };
            if (is_contact(r2))
                return 0.5 * (Inner_Solution() + Outer_Solution());
