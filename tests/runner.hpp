...
extern "C" auto __real___cxa_throw(void* thrown_object, std::type_info* tinfo, void (*dest)(void*)) -> void;

// Wrapper function for __cxa_throw that will be called
//   instead of the original one
extern "C" auto __wrap___cxa_throw(void* thrown_object, std::type_info* tinfo, void (*dest)(void*)) -> void;
...
class safe_runner {
  ...
  public:
    constexpr safe_runner() = default;
    constexpr safe_runner(TReporter reporter, std::size_t suites_size) : reporter_{ std::move(reporter) }, suites_(suites_size) {}
    ...
    auto on(events::test<Ts...> test) {
        ...
        if (filter_(level_, path_)) {
            ...
            try {
                ...
                ++fails_;
                std::string msg = std::string(exception.what()) + "\nStacktrace:\n" + get_stacktrace();
                reporter_.on(events::exception{ msg.c_str() });
            } catch (...) {
                ++fails_;
...
template<>
inline auto cfg<override> = safe_runner<reporter_junit<printer>>{};
} // namespace boost::inline ext::ut::inline v2_3_1