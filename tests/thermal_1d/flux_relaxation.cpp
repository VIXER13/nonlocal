...
T calculate_max_error(const mesh_1d<T>& mesh, const Eigen::Matrix<T, Eigen::Dynamic, 1>& temperature,
...
T solve_thermal_problem_with_relaxation(const std::shared_ptr<mesh_1d<T>>& mesh, const parameters_1d<T>& parameters,
    ...
    for (const size_t step : std::ranges::iota_view{ 1u, Steps }) {
        ...
        boundaries_conditions = { std::make_unique<temperature_1d<T>>(exact_solution(time, 0)),
...
const suite<"thermal_nonstationary_1d_relaxation"> _ = [] {
    ...
    const auto mesh = std::make_shared<mesh_1d<T>>(metamath::finite_element::make_element_1d_integrated<T>(Order, Order),
    ...
    "exact_solution"_test = [&mesh] {
        ...
        static constexpr auto right_part = [](const T t, const T x) constexpr noexcept {
            return -4 + 4 * std::exp(-t) + 4 * t - 2 * t * t + 2 * t * x * x;
        };
        const parameters_1d<T> parameters = {
            { .physical = { .conductivity = T{ 1 }, .capacity = T{ 1 }, .density = T{ 1 }, .relaxation_time = T{ 1 } } }
        };
