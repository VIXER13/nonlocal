...
class cli {
  ...
  public:
    ...
    void set_default_parser() {
      if constexpr (std::is_same_v<T, std::string>) {
        ...
        _parser = [this](std::string_view repr) {
          if (repr == "true" || repr == "1" || repr == "yes") _value = true;
          else if (repr == "false" || repr == "0" || repr == "no") _value = false;
          else throw std::runtime_error("Unsupported boolean repr");
        };
      } else if constexpr (std::is_integral_v<T> || std::is_floating_point_v<T>) {
        _parser = [this](std::string_view repr) {
          T val{};
          auto [ptr, ec] = std::from_chars(repr.data(), repr.data() + repr.size(), val);
          if (ec == std::errc{}) _value = val;
    ...
    iarg& dflt(const T& value) {
      if constexpr (std::is_convertible_v<T, std::string>) _value = std::string(value);
      else _value = value;
    ...
    arg_proxy<T> type() {
      ...
      if (!_parser) set_default_parser<T>();
  ...
  struct arg_proxy {
    const iarg& _arg;
    const T& operator*() { return std::any_cast<const T&>(_arg._value); }
    bool has_value() { return _arg._value.has_value(); }
  };
