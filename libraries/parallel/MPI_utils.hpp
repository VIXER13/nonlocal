...
class MPI_ranges final {
  ...
  public:
    ...
    MPI_ranges& operator=(const MPI_ranges&) = default;
    MPI_ranges& operator=(MPI_ranges&&) = default;
...
Vector all_to_all(const Vector& sendbuf, const MPI_ranges& ranges) {
    ...
    std::vector<int> sendcounts(MPI_size(), data_size * ranges.get().size()),
        sdispls(sendcounts.size(), data_size * ranges.get().front()), recvcounts(sendcounts.size()), rdispls(sendcounts.size());
