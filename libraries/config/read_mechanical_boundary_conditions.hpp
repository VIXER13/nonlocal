...
enum class mechanical_boundary_condition_t : uint8_t { Undefined, Displacement, Pressure, Spring, Combined };

NLOHMANN_JSON_SERIALIZE_ENUM(mechanical_boundary_condition_t, { { mechanical_boundary_condition_t::Undefined, nullptr },
...
class _mechanical_boundary_conditions final {
    ...
    template<std::floating_point T>
    static std::unique_ptr<solver_1d::mechanical::mechanical_boundary_condition_1d<T>> read_mechanical_boundary_condition_1d(
        const nlohmann::json& config, const std::string& path);

    template<std::floating_point T>
    static std::unique_ptr<solver_2d::mechanical::mechanical_boundary_condition_2d<T>> read_mechanical_boundary_condition_2d(
        const nlohmann::json& config, const std::string& path);

    template<std::floating_point T>
    static solver_2d::mechanical::mechanical_boundary_conditions_2d<T> read_mechanical_boundary_conditions_2d(
        const nlohmann::json& config, const std::string& path);
  ...
  public:
    template<std::floating_point T>
    friend solver_1d::mechanical::mechanical_boundaries_conditions_1d<T> read_mechanical_boundaries_conditions_1d(
        const nlohmann::json& config, const std::string& path);

    template<std::floating_point T>
    friend solver_2d::mechanical::mechanical_boundaries_conditions_2d<T> read_mechanical_boundaries_conditions_2d(
        const nlohmann::json& config, const std::string& path);
};
...
_mechanical_boundary_conditions::read_mechanical_boundary_condition_1d(const nlohmann::json& config, const std::string& path) {
    ...
    case mechanical_boundary_condition_t::Combined: {
        ...
        check_parameters(stiffness, path_with_access);
        return std::make_unique<combined_loading_1d<T>>(config.value("pressure", T{ 0 }), stiffness,
                                                        config.value("displacement", T{ 0 }));
    }
...
_mechanical_boundary_conditions::read_mechanical_boundary_condition_2d(const nlohmann::json& config, const std::string& path) {
    ...
    const std::string path_with_access = append_access_sign(path);
    if (has_pressure)
        return std::make_unique<solver_2d::mechanical::pressure_2d<T>>(
            read_coefficient<T, 2u>(config["pressure"], path_with_access + "pressure"));
    return std::make_unique<solver_2d::mechanical::displacement_2d<T>>(
        read_coefficient<T, 2u>(config["displacement"], path_with_access + "displacement"));
}
...
_mechanical_boundary_conditions::read_mechanical_boundary_conditions_2d(const nlohmann::json& config, const std::string& path) {
    ...
    if (!config.is_array() || config.size() != Dimension)
        throw std::domain_error{ "The dimension of the boundary condition \"" + path +
                                 "\" does not correspond to the dimension of the problem" };
...
template<std::floating_point T>
solver_1d::mechanical::mechanical_boundaries_conditions_1d<T> read_mechanical_boundaries_conditions_1d(
    const nlohmann::json& config, const std::string& path) {
    const std::string path_with_access = append_access_sign(path);
    check_required_fields(config, { "left", "right" }, path_with_access);
    using _base = _mechanical_boundary_conditions;
    return { _base::read_mechanical_boundary_condition_1d<T>(config["left"], path_with_access + "left"),
             _base::read_mechanical_boundary_condition_1d<T>(config["right"], path_with_access + "right") };
}

template<std::floating_point T>
solver_2d::mechanical::mechanical_boundaries_conditions_2d<T> read_mechanical_boundaries_conditions_2d(
    const nlohmann::json& config, const std::string& path) {
    const std::string path_with_access = append_access_sign(path);
    ...
    for (const auto& [name, conditions] : config.items())
        boundaries_conditions[name] =
            _mechanical_boundary_conditions::read_mechanical_boundary_conditions_2d<T>(conditions, path_with_access + name);
