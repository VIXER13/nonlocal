...
enum class thermal_boundary_condition_t : uint8_t { Undefined, Temperature, Flux, Convection, Radiation, Combined };

NLOHMANN_JSON_SERIALIZE_ENUM(thermal_boundary_condition_t, { { thermal_boundary_condition_t::Undefined, nullptr },
...
class _read_thermal_boundary_conditions final {
    ...
    template<std::floating_point T>
    static std::unique_ptr<solver_1d::thermal::thermal_boundary_condition_1d<T>> read_thermal_boundary_condition_1d(
        const nlohmann::json& config, const std::string& path);

    template<std::floating_point T>
    static solver_2d::thermal::thermal_boundary_condition_2d<T> read_thermal_boundary_condition_2d(const nlohmann::json& config,
  ...
  public:
    template<std::floating_point T>
    friend solver_1d::thermal::thermal_boundaries_conditions_1d<T> read_thermal_boundaries_conditions_1d(
        const nlohmann::json& config, const std::string& path);

    template<std::floating_point T>
    friend solver_2d::thermal::thermal_boundaries_conditions_2d<T> read_thermal_boundaries_conditions_2d(
        const nlohmann::json& config, const std::string& path);
};

template<std::floating_point T>
void _read_thermal_boundary_conditions::check_parameters(const T heat_transfer, const T emissivity,
...
_read_thermal_boundary_conditions::read_thermal_boundary_condition_1d(const nlohmann::json& config, const std::string& path) {
    ...
    case thermal_boundary_condition_t::Combined: {
        ...
        check_parameters(heat_transfer, emissivity, path_with_access);
        return std::make_unique<combined_flux_1d<T>>(config.value("flux", T{ 0 }), heat_transfer,
                                                     config.value("temperature", T{ 0 }), emissivity);
    }
...
template<std::floating_point T>
solver_2d::thermal::thermal_boundary_condition_2d<T> _read_thermal_boundary_conditions::read_thermal_boundary_condition_2d(
    const nlohmann::json& config, const std::string& path) {
    ...
    case thermal_boundary_condition_t::Temperature:
        check_required_fields(config, { "temperature" }, path_with_access);
        return std::make_unique<temperature_2d<T>>(
            read_coefficient<T, 2u>(config["temperature"], path_with_access + "temperature"));
    ...
    case thermal_boundary_condition_t::Convection: {
        ...
        static constexpr T emissivity = T{ 0 };
        check_parameters(heat_transfer, emissivity, path_with_access);
        return std::make_unique<convection_2d<T>>(
            heat_transfer, read_coefficient<T, 2u>(config["temperature"], path_with_access + "temperature"));
    }
    ...
    case thermal_boundary_condition_t::Combined: {
        ...
        return std::make_unique<combined_flux_2d<T>>(
            config.contains("flux") ? read_coefficient<T, 2u>(config["flux"], path_with_access + "flux") : T{ 0 }, heat_transfer,
            config.contains("temperature") ? read_coefficient<T, 2u>(config["temperature"], path_with_access + "temperature")
                                           : T{ 0 },
            emissivity);
    }
...
template<std::floating_point T>
solver_1d::thermal::thermal_boundaries_conditions_1d<T> read_thermal_boundaries_conditions_1d(const nlohmann::json& config,
                                                                                              const std::string& path) {
    const std::string path_with_access = append_access_sign(path);
    check_required_fields(config, { "left", "right" }, path_with_access);
    using _base = _read_thermal_boundary_conditions;
    return { _base::read_thermal_boundary_condition_1d<T>(config["left"], path_with_access + "left"),
             _base::read_thermal_boundary_condition_1d<T>(config["right"], path_with_access + "right") };
}

template<std::floating_point T>
solver_2d::thermal::thermal_boundaries_conditions_2d<T> read_thermal_boundaries_conditions_2d(const nlohmann::json& config,
                                                                                              const std::string& path) {
    const std::string path_with_access = append_access_sign(path);
