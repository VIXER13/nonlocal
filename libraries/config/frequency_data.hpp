...
enum class frequency_units_t : uint8_t { Hz, KHz, MHz, GHz, Unknown };

NLOHMANN_JSON_SERIALIZE_ENUM(frequency_units_t, { { frequency_units_t::Unknown, nullptr },
...
class frequency_data final {
  ...
  private:
    ...
    T frequency_units_multiplier(const frequency_units_t units) {
        switch (units) {
        case frequency_units_t::Hz:
            return T{ 1e0 };
        case frequency_units_t::KHz:
            return T{ 1e3 };
        case frequency_units_t::MHz:
            return T{ 1e6 };
        case frequency_units_t::GHz:
            return T{ 1e9 };
        case frequency_units_t::Unknown:
        default:
            throw std::domain_error{ "Unsupported frequency units." };
        }
    }
  ...
  public:
    ...
    explicit frequency_data(const nlohmann::json& config, const std::string& path = {}) {
        ...
        if (config.contains("min_value") and config.contains("max_value") and config.contains("number_of_points")) {
            ...
            std::generate_n(
                std::back_inserter(frequencies), number_of_points + 1,
                [current = min_frequency, frequency_step]() mutable { return std::exchange(current, current + frequency_step); });
