...
enum class order_t : uint8_t { Unknown, Linear, Quadratic, Cubic, Quartic, Quintic };

NLOHMANN_JSON_SERIALIZE_ENUM(order_t, { { order_t::Unknown, nullptr },
...
class _read_mesh_1d final {
    ...
    static order_t get_order(const nlohmann::json& config, const std::string& field,
                             const order_t default_order = order_t::Linear);
...
_read_mesh_1d::finite_element_1d<T> _read_mesh_1d::read_element(const nlohmann::json& config, const std::string& path) {
    ...
    if (quadrature_order < element_order)
        logger::warning()
            << "The order of the quadrature is lower than the order of the element, this may lead to computational problems."
            << std::endl;
...
mesh::segment_data<T> _read_mesh_1d::read_segment(const nlohmann::json& config, const std::string& path) {
    ...
    return { .length = config["length"].get<T>(),
