...
struct thermal_auxiliary_data_1d final {
    ...
    operator nlohmann::json() const {
        return { { "energy", energy }, { "right_part", right_part }, { "initial_distribution", initial_distribution } };
    }
};

template<std::floating_point T>
solver_2d::thermal::stationary_equation_parameters_2d<T> read_stationary_equation_parameters_2d(const nlohmann::json& config,
                                                                                                const std::string& path = {}) {
    check_optional_fields(config, { "right_part", "initial_distribution", "energy", "tolerance", "max_iterations" },
                          append_access_sign(path));
