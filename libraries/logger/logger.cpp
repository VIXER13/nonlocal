...
struct dummy_stream : public logger::stream_base {
    ...
    explicit dummy_stream() : logger::stream_base{ empty } {}
};
...
stream_base::stream_base(std::ostream& out) noexcept : out{ out } {}

logger::logger(std::unique_ptr<stream_base>&& out) : _out{ std::move(out) } {}
...
std::optional<level> parse_level(std::string_view name) {
    ...
    if (compare(name, "off"))
        return level::Off;
    if (compare(name, "error"))
        return level::Error;
    if (compare(name, "warning"))
        return level::Warning;
    if (compare(name, "info"))
        return level::Info;
    if (compare(name, "debug"))
        return level::Debug;
    if (compare(name, "trace"))
        return level::Trace;
    return std::nullopt;
}

logger& get(const level level, std::unique_ptr<stream_base>&& init) {
    static constexpr auto levels = std::array{ "OFF: ", "ERROR: ", "WARNING: ", "INFO: ", "DEBUG: ", "TRACE: " };
...
cout_stream::cout_stream() noexcept : stream_base{ std::cout } {}

cerr_stream::cerr_stream() noexcept : stream_base{ std::cerr } {}

file_stream::file_stream(const std::filesystem::path& path) : stream_base{ file }, file{ path, std::ofstream::out } {}
