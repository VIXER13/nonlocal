...
const std::unordered_map<std::string, typename math_expression<T>::unary_operator>& math_expression<T>::unary_operators() {
    ...
    static const std::unordered_map<std::string, typename math_expression<T>::unary_operator> operators{
        { "~", unary_minus },      { "sign", sign },        { "abs", std::abs },     { "sqr", sqr },
        { "sqrt", std::sqrt },     { "cbrt", std::cbrt },   { "sin", std::sin },     { "cos", std::cos },
        { "tan", std::tan },       { "asin", std::asin },   { "acos", std::acos },   { "atan", std::atan },
        { "sinh", std::sinh },     { "cosh", std::cosh },   { "tanh", std::tanh },   { "asinh", std::asinh },
        { "acosh", std::acosh },   { "atanh", std::atanh }, { "exp", std::exp },     { "exp2", std::exp2 },
        { "expm1", std::expm1 },   { "log", std::log },     { "log2", std::log2 },   { "log1p", std::log1p },
        { "log10", std::log10 },   { "erf", std::erf },     { "erfc", std::erfc },   { "tgamma", std::tgamma },
        { "lgamma", std::lgamma }, { "ceil", std::ceil },   { "trunc", std::trunc }, { "floor", std::floor },
        { "round", std::round }
    };
    return operators;
}
...
const std::unordered_map<std::string, typename math_expression<T>::binary_operator>& math_expression<T>::binary_operators() {
    ...
    static const std::unordered_map<std::string, math_expression<T>::binary_operator> operators{
        ...
        { "^", std::pow },     { "pow", pow }, { "atan2", std::atan2 }, { "hypot", std::hypot },
            ...
            if (operator_priority.contains(token.str)) {
                ...
                while (!operators.empty() && operators.top().type != utils::token_t::type_t::ParenthesisLeft) {
                    ...
                    const bool should_pop =
                        (top_priority > token_priority) || (top_priority == token_priority && !token_is_right_associative);
