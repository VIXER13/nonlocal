...
constexpr auto make_lut(std::string_view chars) {
    ...
    for (const auto c : chars)
        lut[static_cast<uint8_t>(c)] = true;
    return lut;
}
    ...
    for (auto it = input.begin(); state != state_t::Terminate;) {
        ...
        case state_t::RealNumber: {
            if (c == 'e' || c == 'E') {
                ...
                throw std::domain_error{ "Wrong expression format. The expression contains invalid dots. Dots are only allowed "
...
const std::unordered_map<std::string, std::size_t>& get_operator_priority() {
    static const std::unordered_map<std::string, size_t> operator_priority{
        { "(", 0 },      { "+", 1 },    { "-", 1 },     { "*", 2 },     { "/", 2 },      { "^", 3 },     { "pow", 3 },
        { "~", 4 },      { "sin", 4 },  { "cos", 4 },   { "tan", 4 },   { "atan", 4 },   { "exp", 4 },   { "abs", 4 },
        { "sign", 4 },   { "sqr", 4 },  { "sqrt", 4 },  { "log", 4 },   { "tgamma", 4 }, { "exp2", 4 },  { "expm1", 4 },
        { "log10", 4 },  { "log2", 4 }, { "log1p", 4 }, { "cbrt", 4 },  { "asin", 4 },   { "acos", 4 },  { "sinh", 4 },
        { "cosh", 4 },   { "tanh", 4 }, { "asinh", 4 }, { "acosh", 4 }, { "atanh", 4 },  { "erf", 4 },   { "erfc", 4 },
        { "lgamma", 4 }, { "ceil", 4 }, { "floor", 4 }, { "round", 4 }, { "trunc", 4 },  { "atan2", 4 }, { "hypot", 4 },
        { "fmod", 4 },   { "min", 4 },  { "max", 4 }
    };
