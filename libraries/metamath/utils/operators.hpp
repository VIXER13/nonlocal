...
void _operators::check_containers(const Container& lhs, const Container& rhs) {
    ...
    if (lhs.size() != rhs.size())
        throw std::logic_error{ "Error in " + operation_name + ". " + "Containers sizes do not match: lhs.size() == " +
...
Container& _operators::sum_assignment(Container& lhs, const Container& rhs) {
    ...
    std::transform(lhs.begin(), lhs.end(), rhs.cbegin(), lhs.begin(), [operation = Operation{}](auto& lhs, const auto& rhs) {
        if constexpr (std::ranges::random_access_range<decltype(lhs)> && std::ranges::random_access_range<decltype(rhs)>)
...
Container& _operators::product_assignment(Container& container, const T& value) {
    std::transform(container.begin(), container.end(), container.begin(), [operation = Operation{}, &value](auto& lhs) {
