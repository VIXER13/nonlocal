...
constexpr bool is_positive(const square_matrix<T, N>& matrix) noexcept {
    if constexpr (N == 1)
        ...
        return matrix[0][0] > T{ 0 } && determinant<T, 2>({ matrix[0][0], matrix[0][1], matrix[1][0], matrix[1][1] }) > T{ 0 } &&
...
template<std::floating_point T, size_t Rows, size_t Cols>
constexpr fixed_matrix<T, Rows, Cols> operator+=(fixed_matrix<T, Rows, Cols>& lhs,
                                                 const fixed_matrix<T, Rows, Cols>& rhs) noexcept {
    for (const size_t row : std::ranges::iota_view{ 0zu, Rows })
        for (const size_t col : std::ranges::iota_view{ 0zu, Cols })
...
constexpr fixed_matrix<T, Rows, Cols> operator*(const fixed_matrix<T, Rows, K>& lhs,
                                                const fixed_matrix<T, K, Cols>& rhs) noexcept {
...
template<std::floating_point T, size_t Rows, size_t Cols>
constexpr fixed_matrix<T, Rows, Cols>& operator*=(fixed_matrix<T, Rows, Cols>& lhs,
                                                  const fixed_matrix<T, Cols, Cols>& rhs) noexcept {
    lhs = lhs * rhs;
    return lhs;
}
