...
enum class matrix_part : bool { Upper, Lower };
...
struct self_adjoint_view final {
    ...
    explicit self_adjoint_view(const sparse_matrix<T, I, J>& matrix) : matrix{ matrix } {
        if (matrix.rows() != matrix.cols())
            throw std::invalid_argument{ "Matrix must be square for self-adjoint view. " + std::to_string(matrix.rows()) + "x" +
                                         std::to_string(matrix.cols()) + " matrix is not square." };
    }
};
