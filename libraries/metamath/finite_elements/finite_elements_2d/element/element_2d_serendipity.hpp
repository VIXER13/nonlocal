...
class element_2d_serendipity : public element_2d_base<T>, public derivative_element_basis_2d<T, 3, Element_Type, Args...> {
  ...
  public:
    ...
    std::unique_ptr<element_2d_base<T>> copy() const override {
        return std::make_unique<element_2d_serendipity>(*this);
    }
    ...
    T N(const size_t i, const std::array<T, 2>& xi) const override {
        return derivative_base::N[i]({ xi[0], xi[1], _p });
    }
    T Nxi(const size_t i, const std::array<T, 2>& xi) const override {
        return derivative_base::Nxi[i]({ xi[0], xi[1], _p });
    }
    T Neta(const size_t i, const std::array<T, 2>& xi) const override {
        return derivative_base::Neta[i]({ xi[0], xi[1], _p });
    }
