...
class element_2d_integrate : public element_integrate_base<T> {
  ...
  public:
    ...
    explicit element_2d_integrate(std::unique_ptr<element_2d_base<T>> element, const quadrature_1d_base<T>& quadrature_x,
    ...
    const quadrature_2d_base<T>& quadrature() const noexcept {
        return *_quadrature;
    }

    T qNxi(const size_t i, const size_t q) const noexcept {
        return _qNxi[i * qnodes_count() + q];
    }
    T qNeta(const size_t i, const size_t q) const noexcept {
        return _qNeta[i * qnodes_count() + q];
    }
    ...
    void set_quadrature(const quadrature_1d_base<T>& quadrature_x, const quadrature_1d_base<T>& quadrature_y) {
        ...
        for (size_t i = 0; i < quadrature_x.nodes_count(); ++i) {
            x[i] =
                element().boundary(side_2d::LEFT, 0) + (quadrature_x.node(i) - quadrature_x.boundary(side_1d::LEFT)) * jacobian_x;
            ...
            for (size_t j = 0; j < quadrature_y.nodes_count(); ++j) {
                y[j] = element().boundary(side_2d::DOWN, x[i]) +
                       (quadrature_y.node(j) - quadrature_y.boundary(side_1d::LEFT)) * jacobian_y[i];
                _weights[i * quadrature_y.nodes_count() + j] =
                    quadrature_x.weight(i) * jacobian_x * quadrature_y.weight(j) * jacobian_y[i];
            }
        }
