...
enum class geometry_t : uint8_t { Rectangle, Triangle };
...
std::unique_ptr<quadrature_2d_base<T>> make_barycentric_quadrature_2d(const size_t order) {
    switch (order) {
    case 1:
        return make_barycentric_quadrature_2d<T, 1>();
    case 2:
        return make_barycentric_quadrature_2d<T, 2>();
    case 3:
        return make_barycentric_quadrature_2d<T, 3>();
    case 4:
        return make_barycentric_quadrature_2d<T, 4>();
    case 5:
        return make_barycentric_quadrature_2d<T, 5>();
    case 6:
        return make_barycentric_quadrature_2d<T, 6>();
    default:
...
std::unique_ptr<quadrature_2d_base<T>> make_quadrature_2d(const size_t order_x, const size_t order_y) {
    return std::make_unique<quadrature_2d<T, cartesian_production>>(*make_quadrature_1d<T>(order_x),
                                                                    *make_quadrature_1d<T>(order_y));
}
