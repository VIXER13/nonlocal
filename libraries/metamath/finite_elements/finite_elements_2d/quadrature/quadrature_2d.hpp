...
class quadrature_2d : public quadrature_2d_base<T>, public Quadrature_Type<T, Args...> {
    using quadrature_t = Quadrature_Type<T, Args...>;
    static_assert(quadrature_t::nodes.size() == quadrature_t::weights.size(), "Number of nodes must match number of weights");
  ...
  public:
    ...
    size_t nodes_count() const override {
        return quadrature_t::nodes.size();
    }

    const std::array<T, 2>& node(const size_t i) const override {
        return quadrature_t::nodes[i];
    }
    T weight(const size_t i) const override {
        return quadrature_t::weights[i];
    }
    T boundary(const side_2d bound, const T x) const override {
        return quadrature_t::boundary(bound, x);
    }
};

template<std::floating_point T>
class quadrature_2d<T, cartesian_production> : public quadrature_2d_base<T>, public cartesian_production<T> {
