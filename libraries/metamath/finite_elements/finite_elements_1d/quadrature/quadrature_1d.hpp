...
class quadrature_1d : public quadrature_1d_base<T>, public Quadrature_Type<T, Args...> {
    using quadrature_t = Quadrature_Type<T, Args...>;
    static_assert(quadrature_t::nodes.size() == quadrature_t::weights.size(), "The number of nodes and weights does not match.");
  ...
  public:
    ...
    T boundary(const side_1d bound) const override {
        return quadrature_t::boundary(bound);
    }
};
