...
class erf_expression : public unary_expression<E, erf_expression, T> {
  ...
  public:
    constexpr explicit erf_expression(const expression<E>& e) noexcept : _base{ e() } {}
    ...
    constexpr auto derivative() const noexcept {
        return integral_constant<2>{} * std::numbers::inv_sqrtpi_v<decltype(T)> * _base::expr().template derivative<X>() *
               exp(-power<2>(_base::expr()));
    }
};
