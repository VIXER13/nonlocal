...
std::vector<Output<T, 2>> approx_in_all_quad_nodes(const mesh_container_2d<T, I>& mesh, const std::vector<I>& qshifts,
...
std::vector<std::array<T, 2>> approx_all_quad_nodes(const mesh_container_2d<T, I>& mesh, const std::vector<I>& qshifts) {
    return approx_in_all_quad_nodes<std::array>(
        mesh, qshifts, [](const auto& element_data, const size_t q) { return element_data.quad_coord(q); });
}

template<class T, class I>
std::vector<metamath::linear::square_matrix<T, 2>> approx_all_jacobi_matrices(const mesh_container_2d<T, I>& mesh,
                                                                              const std::vector<I>& qshifts) {
    return approx_in_all_quad_nodes<metamath::linear::square_matrix>(
        mesh, qshifts, [](const auto& element_data, const size_t q) { return element_data.jacobi_matrix(q); });
}
...
template<class T, class I>
std::vector<std::array<T, 2>> derivatives_in_quad(const mesh_container_2d<T, I>& mesh, const std::vector<I>& quad_element_shifts,
        ...
        for (const size_t i : std::ranges::iota_view{ 0u, el.nodes_count() })
            for (const size_t q : std::ranges::iota_view{ 0u, el.qnodes_count() }) {
                ...
                derivatives[quad_nodes_shifts[e] + i * el.qnodes_count() + q] = {
                    el.qNxi(i, q) * J[1][1] - el.qNeta(i, q) * J[1][0], -el.qNxi(i, q) * J[0][1] + el.qNeta(i, q) * J[0][0]
                };
            }
    }
    return derivatives;
...
void save_as_vtk(Stream& stream, const mesh_container_2d<T, I>& mesh) {
    static constexpr auto write_element = []<size_t K0, size_t... K>(Stream& stream, const std::vector<I>& element,
                                                                     const std::index_sequence<K0, K...>) {
        stream << element[K0];
        ((stream << ' ' << element[K]), ...);
    };
...
void save_vectors_to_vtk(std::ofstream& output, const std::string_view name,
                         const std::vector<std::array<T, Dimension>>& vector) {
