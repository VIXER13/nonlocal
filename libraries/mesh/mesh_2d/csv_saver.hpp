...
using data_pair_t =
    std::variant<std::pair<std::string, std::reference_wrapper<const std::vector<T>>>,
            ...
            [&mesh](const auto& data) {
                if (const auto& [name, vec] = data; vec.get().size() != mesh.nodes_count())
                    throw std::logic_error{ "The result cannot be saved to csv because the mesh nodes number "
                                            ...
                                            to_string(name) + "\" do not match." };
            },
            pair);
}

template<std::floating_point T, std::integral I>
void save_as_csv(const std::filesystem::path& path, const mesh_container_2d<T, I>& mesh, const std::vector<data_pair_t<T>>& data,
