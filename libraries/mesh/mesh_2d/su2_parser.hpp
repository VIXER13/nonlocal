...
template<class T, class I>
mesh_parser<T, I, mesh_format::SU2>::mesh_parser(mesh_container_2d<T, I>& mesh) noexcept : _mesh{ mesh } {}
...
std::unordered_set<std::string> mesh_parser<T, I, mesh_format::SU2>::read_elements(Stream& mesh_file, const size_t count) {
    ...
    for (const size_t e : std::ranges::iota_view{ 0u, count }) {
        ...
        while (std::isspace(element.back()))
            element.pop_back(); // trim string
...
template<class T, class I>
template<class Stream>
std::unordered_map<std::string, std::unordered_set<std::string>> mesh_parser<T, I, mesh_format::SU2>::read_elements_groups(
    Stream& mesh_file) {
    std::string _;
...
template<class T, class I>
size_t mesh_parser<T, I, mesh_format::SU2>::read_group(const std::string& group, const std::unordered_set<std::string>& elements,
                                                       size_t element_shift) {
    ...
    for (const auto& element : elements) {
        ...
        _mesh._elements_types[element_shift] =
            elements_set.is_element_1d(_mesh._elements_types[element_shift])
...
void mesh_parser<T, I, mesh_format::SU2>::parse(Stream& mesh_file) {
    ...
    if (elements_groups.contains(Default_Group_Name.data()))
        throw std::domain_error{ "The group name cannot be named \""s + Default_Group_Name.data() +
                                 "\", as it is a reserved group name." };
