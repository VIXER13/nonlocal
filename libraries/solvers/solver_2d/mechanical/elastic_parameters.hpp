...
namespace isotropic_indices {
enum : uint8_t { _11, _12, _66 };
}
namespace orthotropic_indices {
enum : uint8_t { _11, _12, _66, _22 };
}
namespace anisotropic_indices {
enum : uint8_t { _11, _12, _66, _22, _16, _26 };
}
...
template<class T>
using hooke_matrix_t = std::variant<isotropic_hook_matrix_t<T>, orthotropic_hook_matrix_t<T>, anisotropic_hook_matrix_t<T>>;
...
template<std::floating_point T>
using raw_hook_matrix_t =
    std::variant<raw_isotropic_hook_matrix_t<T>, raw_orthotropic_hook_matrix_t<T>, raw_anisotropic_hook_matrix_t<T>>;
...
template<std::floating_point T>
using evaluated_hook_matrix_t = std::variant<evaluated_isotropic_hook_matrix_t<T>, evaluated_orthotropic_hook_matrix_t<T>,
...
struct anisotropic_elastic_parameters final {
    ...
    static anisotropic_hook_matrix_t<T> rotate(const orthotropic_hook_matrix_t<T>& matrix, const T angle) noexcept {
        ...
        return { main_part_plus + T{ 0.5 } * (matrix[_12] + matrix[_22] + cos_diff) + matrix[_66],
...
template<std::floating_point T>
using elastic_parameters_t =
    std::variant<isotropic_elastic_parameters<T>, orthotropic_elastic_parameters<T>, anisotropic_elastic_parameters<T>>;
...
std::array<T, 3> calc_stress(const isotropic_hook_matrix_t<T>& hooke, const std::array<T, 3>& strain) noexcept {
    ...
    return { hooke[_11] * strain[XX] + hooke[_12] * strain[YY], hooke[_12] * strain[XX] + hooke[_11] * strain[YY],
...
std::array<T, 3> calc_stress(const orthotropic_hook_matrix_t<T>& hooke, const std::array<T, 3>& strain) noexcept {
    using namespace orthotropic_indices;
    return { hooke[_11] * strain[XX] + hooke[_12] * strain[YY], hooke[_12] * strain[XX] + hooke[_22] * strain[YY],
             2 * hooke[_66] * strain[XY] };
}
