        ...
        physical.thermal_strain = std::visit(metamath::types::visitor{
            ...
            [&mesh, &name, &temperature](const raw_orthotropic_thermal_expansion_t<T>& thermal_expansion) -> evaluated_thermal_strain_t<T> {
                ...
                if (is_constant(thermal_expansion)) {
                    orthotropic_constant_thermal_strain<T> result{
                        .delta_temperature = metamath::types::vector_with_shifted_index<T>{std::vector<T>(qshifts.size(), T{0}), qshifts.front()},
                        .thermal_expansion = evaluate<T, 2u>(thermal_expansion, {}, {})
                    };
                    for(const size_t qshift : qshifts)
                        result.delta_temperature[qshift] = temperature[qshift];
                    return result;
                }
                using namespace metamath::functions;
                orthotropic_thermal_strain<T> result{ .strain = {std::vector<std::array<T, 2>>(qshifts.size()), qshifts.front()} };
                for(const size_t qshift : qshifts)
                    result.strain[qshift] = temperature[qshift] * evaluate<T, 2u>(thermal_expansion, mesh.quad_coord(qshift), {});
                return result;
            },
            [&mesh, &name, &temperature](const raw_anisotropic_thermal_expansion_t<T>& thermal_expansion) -> evaluated_thermal_strain_t<T> {
                const auto qshifts = mesh.quad_shifts(name);
                if (is_constant(thermal_expansion)) {
                    anisotropic_constant_thermal_strain<T> result{
                        .delta_temperature = metamath::types::vector_with_shifted_index<T>{std::vector<T>(qshifts.size(), T{0}), qshifts.front()},
                        .thermal_expansion = evaluate<T, 2u>(thermal_expansion, {}, {})
                    };
                    for(const size_t qshift : qshifts)
                ...
                using namespace metamath::functions;
                anisotropic_thermal_strain<T> result{ .strain = {std::vector<std::array<T, 3>>(qshifts.size()), qshifts.front()} };
                for(const size_t qshift : qshifts)
                    result.strain[qshift] = temperature[qshift] * evaluate<T, 2u>(thermal_expansion, mesh.quad_coord(qshift), {});
                return result;
            },
