...
evaluated_mechanical_parameters<T> evaluate_mechanical_parameters(const mesh::mesh_2d<T>& mesh,
    ...
    for (const auto& [name, parameter] : parameters) {
        ...
        physical.density =
            std::visit(metamath::types::visitor{ [](const std::monostate) -> evaluated_density_t<T> { return std::monostate{}; },
                                                 [&mesh, &name](const coefficient_t<T, 2>& density) -> evaluated_density_t<T> {
                                                     if (is_constant(density))
                                                         return evaluate<T, 2u>(density, {}, {});
                                                     const auto qshifts = mesh.quad_shifts(name);
                                                     metamath::types::vector_with_shifted_index<T> result = {
                                                         .container = std::vector<T>(qshifts.size()), .shift = qshifts.front()
                                                     };
                                                     for (const size_t qshift : qshifts)
        ...
        physical.thermal_strain = std::visit(
            metamath::types::visitor{
                [](const std::monostate) -> evaluated_thermal_strain_t<T> { return std::monostate{}; },
                [&mesh, &name,
                 &temperature](const raw_isotropic_thermal_expansion_t<T>& thermal_expansion) -> evaluated_thermal_strain_t<T> {
                    const auto qshifts = mesh.quad_shifts(name);
                    isotropic_thermal_strain<T> result{ .strain = { std::vector<T>(qshifts.size(), T{ 0 }), qshifts.front() } };
                    for (const size_t qshift : qshifts)
                        result.strain[qshift] =
                            temperature[qshift] * evaluate<T, 2u>(thermal_expansion, mesh.quad_coord(qshift), {});
                    return result;
                },
                [&mesh, &name, &temperature](const auto& thermal_expansion) -> evaluated_thermal_strain_t<T> {
                    ...
                    if (is_constant(thermal_expansion)) {
                        std::conditional_t<std::is_same_v<Expansion, raw_orthotropic_thermal_expansion_t<T>>,
                            ...
                            result = { .delta_temperature =
                                           metamath::types::vector_with_shifted_index<T>{
                    ...
                    std::conditional_t<std::is_same_v<Expansion, raw_orthotropic_thermal_expansion_t<T>>,
                                       orthotropic_thermal_strain<T>, anisotropic_thermal_strain<T>>
                        result = { .strain = { .container =
                                                   std::vector<std::array<T, std::tuple_size_v<Expansion>>>(qshifts.size()),
                                               .shift = qshifts.front() } };
                    for (const size_t qshift : qshifts)
                        result.strain[qshift] =
                            temperature[qshift] * evaluate<T, 2u>(thermal_expansion, mesh.quad_coord(qshift), {});
