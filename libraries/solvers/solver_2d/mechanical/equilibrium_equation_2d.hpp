...
auto init_preconditioner(problem_settings settings, const mesh::mesh_2d<T>& mesh,
...
template<std::floating_point T>
mechanical::mechanical_solution_2d<T> equilibrium_equation(
    const std::shared_ptr<mesh::mesh_2d<T>>& mesh, const raw_mechanical_parameters<T>& parameters,
    const mechanical_boundaries_conditions_2d<T>& boundaries_conditions, const std::vector<T>& delta_temperature = {},
    const std::function<std::array<T, 2>(const std::array<T, 2>&)>& right_part = nullptr, const bool use_preconditioner = true) {
