...
problem_settings init_problem_settings(const mesh::mesh_container_2d<T>& mesh, const raw_mechanical_parameters<T>& parameters,
    ...
    static constexpr auto is_nonconstant_parameters = [](const auto& parameter) {
        return !is_constant(parameter.physical.elastic);
    };
