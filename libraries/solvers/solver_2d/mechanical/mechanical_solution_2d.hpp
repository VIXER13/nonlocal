...
mechanical_solution_2d<T>::mechanical_solution_2d(const std::shared_ptr<mesh::mesh_2d<T>>& mesh)
    : _base{ mesh }, _displacement(mesh->container().nodes_count(), std::array<T, 2>{}) {}

template<class T>
mechanical_solution_2d<T>::mechanical_solution_2d(const std::shared_ptr<mesh::mesh_2d<T>>& mesh,
                                                  const evaluated_mechanical_parameters<T>& parameters,
                                                  std::vector<std::array<T, 2>>&& displacement)
    : _base{ mesh, {} }, _displacement(std::move(displacement)) {}
...
void mechanical_solution_2d<T>::substract_temperature_strains(std::vector<std::array<T, 3>>& strain,
    ...
    for (const auto& [group, parameter] : parameters)
        std::visit(metamath::types::visitor{ [](const std::monostate) {},
...
template<class T>
template<class Hooke_Matrix, class Influence>
std::array<T, 3> mechanical_solution_2d<T>::calc_nonlocal_stress(const size_t eL, const Hooke_Matrix& hooke_matrices,
    ...
    for (const size_t eNL : _base::mesh().neighbours(eL)) {
        ...
        for (const size_t qNL : elNL.qnodes()) {
            ...
            const auto& hooke =
                hooke_matrices.index() ? std::get<Variable>(hooke_matrices)[qshift] : std::get<Constant>(hooke_matrices);
            const T influence_weight =
                elNL.weight(qNL) * _base::mesh().jacobian(qshift) * influence(_base::mesh().quad_coord(qshift));
            using namespace metamath::operators;
...
void mechanical_solution_2d<T>::calc_strain_and_stress(const evaluated_mechanical_parameters<T>& parameters) {
    ...
    for (const auto& [group, parameter] : parameters) {
        ...
        std::visit(
            [this, &model = parameter.model, &elements, &strains](const auto& hooke_matrices) {
#pragma omp parallel for schedule(dynamic)
                for (size_t eL = elements.front(); eL < *elements.end(); ++eL)
                    for (const size_t qshiftL :
                         std::ranges::iota_view{ _base::mesh().quad_shift(eL), _base::mesh().quad_shift(eL + 1) }) {
                        using namespace metamath::operators;
                        ...
                        if (theory_type(model.local_weight) == theory_t::NONLOCAL) {
                            ...
                            const auto influence = [nonlocal_weight, &influence = model.influence,
                                                    &qnodeL](const std::array<T, 2>& qnodeNL) {
