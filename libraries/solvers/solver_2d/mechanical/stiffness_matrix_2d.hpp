...
class stiffness_matrix : public matrix_assembler_base<metamath::linear::square_matrix<T, 2>> {
  ...
  protected:
    ...
    metamath::linear::square_matrix<T, 2> integrate_local(const Hooke& hooke_matrix, const size_t e, const size_t i,
...
template<std::floating_point T>
stiffness_matrix<T>::stiffness_matrix(const mesh::mesh_2d<T>& mesh) : _base{ mesh } {}

template<std::floating_point T>
template<class Hooke>
metamath::linear::square_matrix<T, 2> stiffness_matrix<T>::integrate_local(const Hooke& hooke_matrix, const size_t e,
    ...
    for (const size_t q : el.qnodes()) {
        ...
        const auto& hooke =
            hooke_matrix.index() ? std::get<Variable>(hooke_matrix)[qshift + q] : std::get<Constant>(hooke_matrix);
...
metamath::linear::square_matrix<T, 2> stiffness_matrix<T>::integrate_nonlocal(
    ...
    for (const size_t qL : elL.qnodes()) {
        ...
        for (const size_t qNL : elNL.qnodes()) {
            const auto& hooke =
                hooke_matrix.index() ? std::get<Variable>(hooke_matrix)[qshiftNL + qNL] : std::get<Constant>(hooke_matrix);
...
void stiffness_matrix<T>::compute(const evaluated_mechanical_parameters<T>& hooke, const problem_settings& settings) {
    ...
    _base::calc_coeffs(
        ...
        [this, &hooke](const std::string& group, const size_t e, const size_t i, const size_t j) {
            ...
            const auto integral =
                std::visit([this, e, i, j](const auto& hook) { return integrate_local(hook, e, i, j); }, physic.elastic);
            using namespace metamath::operators;
        ...
        [this, &hooke](const std::string& group, const size_t eL, const size_t eNL, const size_t iL, const size_t jNL) {
            const auto& [model, physic] = hooke.at(group);
            const auto integral =
                std::visit([this, &model, eL, eNL, iL,
                            jNL](const auto& hook) { return integrate_nonlocal(hook, model.influence, eL, eNL, iL, jNL); },
                           physic.elastic);
            using namespace metamath::operators;
