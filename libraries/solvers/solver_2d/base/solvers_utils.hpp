...
void run_by_boundary(const mesh::mesh_container_2d<T>& mesh, const std::string& bound_name,
                     const boundary_condition_2d<T, Physics>* const condition, const size_t degree, const Callback& callback) {
...
void run_by_boundaries(const mesh::mesh_container_2d<T>& mesh,
                       const boundaries_conditions_2d<T, Physics, DoF>& boundaries_conditions, const Callback& callback) {
...
std::vector<bool> inner_nodes(const mesh::mesh_container_2d<T>& mesh,
    ...
    run_by_boundaries<first_kind_2d, Physics>(mesh, boundaries_conditions,
                                              [&is_inner](const first_kind_2d<T, Physics>&, const size_t, const size_t node,
                                                          const size_t degree) { is_inner[DoF * node + degree] = false; });
    return is_inner;
}
