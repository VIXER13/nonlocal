...
template<class T, std::integral I, size_t DoF, class Local_Integrator, class Nonlocal_Integrator>
...
public:
    explicit integrator(finite_element_matrix<T>& matrix, const mesh::mesh_container_2d<T, I>& mesh, 
...
integrator<T, I, DoF, Local_Integrator, Nonlocal_Integrator>::integrator(
    finite_element_matrix<T>& matrix, const mesh::mesh_container_2d<T, I>& mesh, 
    const std::vector<bool>& is_inner, const size_t node_shift, const bool is_symmetric,
    const Local_Integrator& local_integrator, const Nonlocal_Integrator& nonlocal_integrator)
