...
protected:
    ...
    explicit indexator_base(const size_t size);
...
template<size_t DoF>
indexator_base<DoF>::indexator_base(const size_t size) {
...
void indexator_base<DoF>::reset(const size_t node) {
    for(const size_t i : std::ranges::iota_view{0u, DoF}) {
        ...
        std::fill(std::next(flags(matrix_part::INNER)[i].begin(), DoF * node), flags(matrix_part::INNER)[i].end(), false);
    }
}
