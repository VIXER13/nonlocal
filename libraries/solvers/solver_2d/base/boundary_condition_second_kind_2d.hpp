...
void boundary_condition_second_kind_2d(std::vector<T>& f, const mesh::mesh_2d<U>& mesh,
    ...
    utils::run_by_boundaries<second_kind_2d, Physics>(
        mesh.container(), boundaries_conditions,
        [&f, &mesh, process_nodes = mesh.process_nodes()](const second_kind_2d<U, Physics>& condition, const size_t be,
                                                          const size_t node, const size_t degree) {
