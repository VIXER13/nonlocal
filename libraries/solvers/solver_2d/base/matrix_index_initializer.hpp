...
class matrix_index_initializer final : public matrix_separator_base<T>
                                     , public matrix_indexator_base<DoF> {
    using _matrix = matrix_separator_base<T>;
...
public:
    explicit matrix_index_initializer(finite_element_matrix<T>& matrix, const mesh::mesh_container_2d<T, I>& mesh, 
...
template<class T, std::integral I, size_t DoF>
matrix_index_initializer<T, I, DoF>::matrix_index_initializer(finite_element_matrix<T>& matrix, const mesh::mesh_container_2d<T, I>& mesh, 
                                                              const std::vector<bool>& is_inner, const size_t node_shift, const bool is_symmetric)
...
template<class T, std::integral I, size_t DoF>
void matrix_index_initializer<T, I, DoF>::reset(const size_t node) {
    for(const matrix_part part : {matrix_part::INNER, matrix_part::BOUND})
        for(const size_t dof : std::ranges::iota_view{0u, DoF})
            _current_indices[size_t(part)][dof] = _matrix::matrix(part).portrait.shifts[DoF * (node - _matrix::node_shift()) + dof];
    _indexator::reset(node);
}

template<class T, std::integral I, size_t DoF>
void matrix_index_initializer<T, I, DoF>::run(const size_t row_glob, const size_t col_glob) {
    for(const size_t row_loc : std::ranges::iota_view{0u, DoF})
        for(const size_t col_loc : std::ranges::iota_view{0u, DoF}) {
            ...
            _indexator::check_flag(part, row_loc, col, [this, part, col, row_loc]() {
                _matrix::matrix(part).portrait.indices[_current_indices[size_t(part)][row_loc]++] = col;
            });
        }
}

template<class T, std::integral I, size_t DoF>
void matrix_index_initializer<T, I, DoF>::operator()(const std::string&, const size_t e, const size_t i, const size_t j) {
    run(DoF * _mesh.node_number(e, i), DoF * _mesh.node_number(e, j));
}

template<class T, std::integral I, size_t DoF>
void matrix_index_initializer<T, I, DoF>::operator()(const std::string&, const size_t eL, const size_t eNL, const size_t iL, const size_t jNL) {
