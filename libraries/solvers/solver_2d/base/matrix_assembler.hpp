...
class matrix_assembler_base {
    ...
    using nodes_sequence = std::variant<std::ranges::iota_view<size_t, size_t>, std::vector<size_t>>;
  ...
  protected:
    ...
    void calc_coeffs(const problem_settings& settings, Local_Integrator&& local_integrator,
                     Nonlocal_Integrator&& nonlocal_integrator);
...
matrix_assembler_base<T>::matrix_assembler_base(const mesh::mesh_2d<floating_point_t>& mesh)
    : _mesh{ mesh }, processing_nodes{ _mesh.process_nodes() } {}
...
void matrix_assembler_base<T>::mesh_run(const problem_settings& settings, Runner&& runner) {
    std::visit([this, &settings,
                &runner](const auto& nodes) { mesh::utils::mesh_run(mesh(), nodes, settings.theories, std::move(runner)); },
               processing_nodes);
}
...
template<class T>
template<class Local_Integrator, class Nonlocal_Integrator>
void matrix_assembler_base<T>::calc_coeffs(const problem_settings& settings, Local_Integrator&& local_integrator,
                                           Nonlocal_Integrator&& nonlocal_integrator) {
    mesh_run(settings, integrator{ matrix(), mesh().container(), settings.is_symmetric(), std::move(local_integrator),
                                   std::move(nonlocal_integrator) });
}
