...
public:
    explicit shift_initializer(finite_element_matrix<T>& matrix, const mesh::mesh_container_2d<T>& mesh, 
...
template<class T, size_t DoF>
shift_initializer<T, DoF>::shift_initializer(finite_element_matrix<T>& matrix, const mesh::mesh_container_2d<T>& mesh, 
                                             const std::vector<bool>& is_inner, const size_t node_shift, const bool is_symmetric)
...
template<class T, size_t DoF>
void shift_initializer<T, DoF>::run(const size_t row_glob, const size_t col_glob) {
...
template<class T, size_t DoF>
void shift_initializer<T, DoF>::operator()(const std::string&, const size_t e, const size_t i, const size_t j) {
    run(DoF * _mesh.node_number(e, i), DoF * _mesh.node_number(e, j));
}

template<class T, size_t DoF>
void shift_initializer<T, DoF>::operator()(const std::string&, const size_t eL, const size_t eNL, const size_t iL, const size_t jNL) {
