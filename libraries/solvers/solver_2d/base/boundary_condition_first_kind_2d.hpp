...
class _boundary_condition_first_kind_2d final {
    ...
    template<size_t DoF, class T, class I, class Conditions_Map>
    static void set_values(Eigen::Matrix<T, Eigen::Dynamic, 1>& right_part,
                           const Eigen::Matrix<T, Eigen::Dynamic, 1>& x,
                           const mesh::mesh_2d<T, I>& mesh,
                           const Conditions_Map& boundaries_conditions) {
        utils::run_by_boundaries<DoF, first_kind_2d>(mesh.container(), boundaries_conditions,
            [&right_part, &x, &mesh](const auto&, const size_t, const size_t node, const size_t degree) {
                if (const auto process_nodes = mesh.process_nodes(); node >= process_nodes.front() && node <= process_nodes.back()) {
                    const size_t index = DoF * node + degree;
                    right_part[index] = x[index];
                }
            });
    }
...
public:
    ...
    friend void boundary_condition_first_kind_2d(Eigen::Matrix<T, Eigen::Dynamic, 1>& right_part,
                                                 const mesh::mesh_2d<T, I>& mesh,
                                                 const Conditions_Map& boundaries_conditions,
...
template<size_t DoF, class T, class I, class Matrix_Index, class Conditions_Map>
void boundary_condition_first_kind_2d(Eigen::Matrix<T, Eigen::Dynamic, 1>& right_part,
                                      const mesh::mesh_2d<T, I>& mesh,
                                      const Conditions_Map& boundaries_conditions,
                                      const Eigen::SparseMatrix<T, Eigen::RowMajor, Matrix_Index>& K_bound) {
    const auto x = _boundary_condition_first_kind_2d::calc_vector<DoF>(mesh.container(), boundaries_conditions);
    right_part -= K_bound * x;
    _boundary_condition_first_kind_2d::set_values<DoF>(right_part, x, mesh, boundaries_conditions);
}
