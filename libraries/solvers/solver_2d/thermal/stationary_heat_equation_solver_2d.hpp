...
template<std::floating_point T>
std::vector<T> init_right_part(const mesh::mesh_2d<T>& mesh, const thermal_boundaries_conditions_2d<T>& boundaries_conditions,
                               const stationary_equation_parameters_2d<T>& auxiliary_data, const bool is_neumann = false) {
...
template<std::floating_point T>
std::unique_ptr<slae::preconditioner_base<T>> init_preconditioner(
    problem_settings settings, const mesh::mesh_2d<T>& mesh, const evaluated_thermal_parameters<T>& parameters,
    const thermal_boundaries_conditions_2d<T>& boundaries_conditions, const std::vector<T>& temperature = {}) {
    ...
    convection_condition_2d(local_conductivity.matrix(), settings, mesh,
                            boundaries_conditions); // TODO: fix computational range for MPI
...
std::vector<T> stationary_heat_equation_solver_2d_linear(const problem_settings& settings,
    ...
    convection_condition_2d(conductivity.matrix(), settings, *mesh, boundaries_conditions);
...
std::vector<T> stationary_heat_equation_solver_2d_nonlinear(const problem_settings& settings,
    ...
    do {
        ...
        const auto conductivity_parameters =
            evaluate_conductivity(*mesh, parameters, mesh::utils::nodes_to_qnodes<T>(*mesh, temperature));
        ...
        if (!settings.is_neumann)
            boundary_condition_first_kind_2d(conductivity.matrix(), right_part, settings, mesh->container(),
        ...
        convection_condition_2d(conductivity.matrix(), settings, *mesh, boundaries_conditions);
        radiation_condition_2d(conductivity.matrix(), settings, *mesh, boundaries_conditions, temperature);
        ...
        delta_temperature = solver->solve(right_part, delta_temperature);
...
heat_equation_solution_2d<T> stationary_heat_equation_solver_2d(const std::shared_ptr<mesh::mesh_2d<T>>& mesh,
    ...
    if (settings.is_nonlinear()) {
        temperature = stationary_heat_equation_solver_2d_nonlinear(settings, mesh, parameters, boundaries_conditions,
                                                                   auxiliary_data, right_part);
        ...
        temperature = stationary_heat_equation_solver_2d_linear(settings, mesh, conductivity_parameters, boundaries_conditions,
                                                                std::move(right_part));
    }
