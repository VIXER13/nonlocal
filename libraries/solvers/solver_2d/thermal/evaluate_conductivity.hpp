...
evaluated_thermal_parameters<T> evaluate_conductivity(const mesh::mesh_2d<T>& mesh, const raw_thermal_parameters<T>& parameters,
    ...
    for (const auto& [name, parameter] : parameters) {
        ...
        param.physical.conductivity = std::visit(
            [&mesh, &name, &solution](const auto& conductivity) -> evaluated_conductivity_t<T> {
                ...
                metamath::types::vector_with_shifted_index<Type> result = { .container = std::vector<Type>(qshifts.size()),
                                                                            .shift = qshifts.front() };
