...
public:
    ...
    void calc_step(const boundaries_conditions_2d<T>& boundaries_conditions,
...
void nonstationary_heat_equation_solver_2d<T, I, Matrix_Index>::compute(const parameter_2d<T>& parameters,
                                                                        ...
                                                                        const Init_Dist& init_dist,
                                                                        const T p1, const Influence_Function& influence_function) {
    const std::vector<bool> is_inner = utils::inner_nodes<DoF>(_conductivity.mesh().container(), boundaries_conditions);
...
template<class T, class I, class Matrix_Index>
template<class Right_Part>
void nonstationary_heat_equation_solver_2d<T, I, Matrix_Index>::calc_step(const boundaries_conditions_2d<T>& boundaries_conditions,
                                                                          const Right_Part& right_part) {
    _right_part.setZero();
    _temperature_prev.swap(_temperature_curr);
    boundary_condition_second_kind_2d<DoF>(_right_part, _conductivity.mesh(), boundaries_conditions);
    ...
    boundary_condition_first_kind_2d<DoF>(_right_part, _conductivity.mesh(), boundaries_conditions, _conductivity.matrix_bound());
