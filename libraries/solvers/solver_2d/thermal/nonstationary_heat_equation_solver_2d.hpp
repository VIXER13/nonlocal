...
void nonstationary_heat_equation_solver_2d<T, I, Matrix_Index>::compute(const parameters_2d<T>& parameters,
    ...
    convection_condition_2d(_conductivity.matrix()[matrix_part::INNER], _conductivity.mesh(), boundaries_conditions);
    _capacity.calc_matrix(parameters, is_inner);

    _conductivity.matrix()[matrix_part::INNER] *= time_step();
    _conductivity.matrix()[matrix_part::BOUND] *= time_step();
    _conductivity.matrix()[matrix_part::INNER] += _capacity.matrix()[matrix_part::INNER];
    first_kind_filler(_conductivity.mesh().process_nodes(), is_inner, [&matrix = _conductivity.matrix()[matrix_part::INNER]](const size_t row) {
        matrix.valuePtr()[matrix.outerIndexPtr()[row]] = T{1};
    });
    _conductivity_initial_matrix_inner = _conductivity.matrix()[matrix_part::INNER];
    ...
    slae_solver = std::make_unique<slae::conjugate_gradient<T, Matrix_Index>>(_conductivity.matrix()[matrix_part::INNER]);
}
...
void nonstationary_heat_equation_solver_2d<T, I, Matrix_Index>::calc_step(const thermal_boundaries_conditions_2d<T>& boundaries_conditions,
    ...
    _conductivity.matrix()[matrix_part::INNER] = _conductivity_initial_matrix_inner;
    radiation_condition_2d(_conductivity.matrix()[matrix_part::INNER], _right_part, _conductivity.mesh(), boundaries_conditions, 
    ...
    _right_part += _capacity.matrix()[matrix_part::INNER].template selfadjointView<Eigen::Upper>() * _temperature_prev;
    boundary_condition_first_kind_2d(_right_part, _conductivity.mesh(), boundaries_conditions, _conductivity.matrix()[matrix_part::BOUND]);
