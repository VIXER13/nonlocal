...
void independent_symmetric_matrix_vector_product<T, I>::reduction(Eigen::Matrix<T, Eigen::Dynamic, 1>& result) const {
    for(const size_t i : std::ranges::iota_view{1u, _thread_rows.size()})
        _threaded_product.block(_thread_rows.get(i).front(), 0, result.size() - _thread_rows.get(i).front(), 1) += 
        _threaded_product.block(_thread_rows.get(i).front(), i, result.size() - _thread_rows.get(i).front(), 1);
