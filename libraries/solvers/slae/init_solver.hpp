...
std::unique_ptr<iterative_solver_base<T, I, J>> init_iterative_solver(const metamath::linear::sparse_matrix<T, I, J>& matrix,
...
template<class T, std::integral I, std::integral J>
std::unique_ptr<preconditioner_base<T>> init_preconditioner(metamath::linear::sparse_matrix<T, I, J>&& matrix,
                                                            const bool is_symmetric) {
    if (is_symmetric)
...
template<class T, std::integral I, std::integral J>
std::unique_ptr<preconditioner_base<T>> init_eigen_preconditioner(metamath::linear::sparse_matrix<T, I, J>&& matrix,
                                                                  const bool is_symmetric) {
    // A temporary solution that uses preconditioners from the Eigen library.
    // The plan is to eliminate this dependency in the future once native versions of the preconditioners are implemented for our
    // data types.
    if (is_symmetric)
