...
class eigen_preconditioner_base : public preconditioner_base<T> {
  ...
  protected:
        ...
        if constexpr (DoF<T> == 1) {
            ...
            for (const size_t s : std::ranges::iota_view{ 0zu, matrix.rows() })
                for (const size_t d : std::ranges::iota_view{ 0zu, DoF<T> })
                    eigen_matrix.outerIndexPtr()[DoF<T> * s + d + 1] =
                        DoF<T> * (matrix.portrait.shifts[s + 1] - matrix.portrait.shifts[s]);
    ...
    explicit eigen_preconditioner_base(const metamath::linear::sparse_matrix<T, I, J>& matrix)
        : _right_part{ EI(DoF<T> * matrix.cols()) }, _solution{ EI(DoF<T> * matrix.cols()) } {
    ...
    std::vector<entity_t> solve(const std::vector<entity_t>& rhs) const override {
        if (rhs.size() != _right_part.size() / DoF<T>)
            throw std::invalid_argument{
                "The size of the right-hand side vector does not match the number of columns in ildlt preconditioner matrix."
            };
...
class ilut_eigen_preconditioner
    : public eigen_preconditioner_base<T, Eigen::IncompleteLUT<typename preconditioner_base<T>::floating_point_t, int64_t>> {
    using _base = eigen_preconditioner_base<T, Eigen::IncompleteLUT<typename preconditioner_base<T>::floating_point_t, int64_t>>;
    using _base::_preconditioner;
    using _base::matrix_to_eigen;

  public:
    explicit ilut_eigen_preconditioner(metamath::linear::sparse_matrix<T, I, J>&& matrix) : _base{ matrix } {
...
template<class T, std::integral I, std::integral J>
class ildlt_eigen_preconditioner
    : public eigen_preconditioner_base<T, Eigen::IncompleteCholesky<typename preconditioner_base<T>::floating_point_t,
                                                                    Eigen::Upper, Eigen::NaturalOrdering<int64_t>>> {
    using _base = eigen_preconditioner_base<T, Eigen::IncompleteCholesky<typename preconditioner_base<T>::floating_point_t,
                                                                         Eigen::Upper, Eigen::NaturalOrdering<int64_t>>>;
    using _base::_preconditioner;
    using _base::matrix_to_eigen;

  public:
    explicit ildlt_eigen_preconditioner(metamath::linear::sparse_matrix<T, I, J>&& matrix) : _base{ matrix } {
