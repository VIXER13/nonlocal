...
enum class product_strategy : bool { INDEPENDENT, UNRELATED };

template<class T, class I, product_strategy Strategy = product_strategy::INDEPENDENT>
class conjugate_gradient final
    : public std::conditional_t<Strategy == product_strategy::INDEPENDENT, independent_symmetric_matrix_vector_product<T, I>,
                                unrelated_symmetric_matrix_vector_product<T, I>> {
    using _base = std::conditional_t<Strategy == product_strategy::INDEPENDENT, independent_symmetric_matrix_vector_product<T, I>,
                                     unrelated_symmetric_matrix_vector_product<T, I>>;
...
Eigen::Matrix<T, Eigen::Dynamic, 1> conjugate_gradient<T, I, Strategy>::solve(
    ...
    Eigen::Matrix<T, Eigen::Dynamic, 1> z = Eigen::Matrix<T, Eigen::Dynamic, 1>::Zero(
        matrix().cols()); // It used as the right part in preparation calculation before iteration process
