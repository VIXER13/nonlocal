...
class assembler_base_1d {
    static constexpr auto Empty_Function = [](const size_t, const size_t, const size_t, const size_t,
                                              const size_t) constexpr noexcept { return T{ 0 }; };
...
assembler_base_1d<T, I>::assembler_base_1d(finite_element_matrix_1d<T, I>& matrix, const std::shared_ptr<mesh::mesh_1d<T>>& mesh,
    ...
    : _matrix{ matrix }, _mesh{ mesh },
      _nodes_to_assemble{ nodes_to_assemble ? *nodes_to_assemble
                                            : std::ranges::iota_view<size_t, size_t>{ 0u, mesh->nodes_count() } } {}
...
void assembler_base_1d<T, I>::mesh_run(const size_t segment, const Callback& callback) const {
    ...
    utils::iterate(
        get_subsequence(segment),
        [this, &correct_node_data, &callback](const size_t node) {
...
void assembler_base_1d<T, I>::calc_matrix(const problem_settings& settings, const Integrate_Loc& integrate_loc,
                                          const Integrate_Nonloc& integrate_nonloc) {
    const auto assemble_bound = [this](std::unordered_map<size_t, T>& matrix_bound, const size_t row, const size_t col,
                                       const T integral) {
        if (col == row)
    ...
    const auto assemble = [this, &settings, &assemble_bound, is_symmetric = settings.is_symmetric(),
                           last_node = mesh().nodes_count() - 1]<class Integrate, class... Model_Args>(
    ...
    for (const size_t segment : mesh().segments())
        ...
        case theory_t::NONLOCAL:
            mesh_run<theory_t::NONLOCAL>(segment, [this, &assemble, &integrate_nonloc, segment](
        ...
        case theory_t::LOCAL:
            mesh_run<theory_t::LOCAL>(
                segment, [this, &assemble, integrate_loc, segment](const size_t e, const size_t i, const size_t j) {
