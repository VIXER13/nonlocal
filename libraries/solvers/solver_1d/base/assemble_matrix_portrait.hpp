...
void init_shifts(Eigen::SparseMatrix<T, Eigen::RowMajor, I>& matrix,
                 ...
                 const std::vector<theory_t>& theories,
                 const std::array<bool, 2> is_first_kind,
                 const bool is_neumann = false) {
    if (theories.size() != mesh.segments_count())
...
#pragma omp parallel for default(none) shared(matrix, mesh, theories, is_first_kind, is_neumann, last_node, nodes_in_element)
    for(size_t node = is_first_kind.front(); node < mesh.nodes_count() - is_first_kind.back(); ++node) {
        ...
        const bool is_right_first_kind = is_first_kind.back() && neighbour * nodes_in_element == last_node;
        const size_t count = (neighbour - e) * nodes_in_element + 1 - i - is_right_first_kind;
        matrix.outerIndexPtr()[node + 1] = count + is_neumann;
    }
...
void init_matrix_portrait(Eigen::SparseMatrix<T, Eigen::RowMajor, I>& matrix,
    ...
    init_shifts(matrix, mesh, theories, is_first_kind, is_neumann);
    init_indices(matrix, is_neumann);
    if (!is_symmetric)
