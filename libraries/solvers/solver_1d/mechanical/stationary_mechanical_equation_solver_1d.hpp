...
mechanical_equation_solution_1d<T> stationary_mechanical_equation_solver_1d(
    const std::shared_ptr<mesh::mesh_1d<T>>& mesh, const parameters_1d<T>& parameters,
    const mechanical_boundaries_conditions_1d<T>& boundaries_conditions,
    const stationary_equation_parameters_1d<T>& additional_parameters) {
    ...
    Eigen::Matrix<T, Eigen::Dynamic, 1> right_part =
        init_right_part(mesh, boundaries_conditions, additional_parameters, settings.is_neumann);
    ...
    do {
        ...
        assembler.calc_matrix(parameters, settings,
                              settings.is_solution_dependent ? std::optional{ from_nodes_to_qnodes(*mesh, displacement_prev) }
                                                             : std::nullopt);

        if (settings.is_neumann) {
            if (settings.is_symmetric()) {
                const Eigen::ConjugateGradient<Eigen::SparseMatrix<T, Eigen::RowMajor, I>, Eigen::Upper> solver{
                    stiffness.inner
                };
            ...
            if (settings.is_symmetric()) {
                const Eigen::SimplicialCholesky<Eigen::SparseMatrix<T, Eigen::RowMajor, I>, Eigen::Upper,
                                                Eigen::NaturalOrdering<I>>
                    solver{ stiffness.inner };
                displacement_curr = solver.solve(right_part);
            } else {
                const Eigen::SparseLU<Eigen::SparseMatrix<T, Eigen::RowMajor, I>, Eigen::NaturalOrdering<I>> solver{
                    stiffness.inner
                };
                displacement_curr = solver.solve(right_part);
            }
    ...
    } while (settings.is_nonlinear() && iteration < additional_parameters.max_iterations &&
