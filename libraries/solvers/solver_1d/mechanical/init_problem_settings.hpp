...
problem_settings init_problem_settings(const parameters_1d<T>& parameters,
    ...
    static constexpr auto is_nonconstant_parameters = [](const auto& parameter) noexcept {
        return std::holds_alternative<spatial_dependency<T, 1>> (parameter.physical.youngs_modulus) ||
               std::holds_alternative<solution_dependency<T, 1>>(parameter.physical.youngs_modulus) ||
               std::holds_alternative<spatial_dependency<T, 1>> (parameter.physical.density) ||
