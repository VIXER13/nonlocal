...
template<class T>
using temperature_function = std::variant<std::function<T(const T)>, std::vector<T>, Eigen::Matrix<T, Eigen::Dynamic, 1>>;
...
class nonstationary_heat_equation_solver_1d final {
    ...
    static void reset_initial_values(Eigen::SparseMatrix<T, Eigen::RowMajor, I>& matrix, const std::array<T, 2>& values);
  ...
  public:
    explicit nonstationary_heat_equation_solver_1d(const std::shared_ptr<mesh::mesh_1d<T>>& mesh,
                                                   const parameters_1d<T>& parameters, const T time_step,
...
template<class T, class I>
nonstationary_heat_equation_solver_1d<T, I>::nonstationary_heat_equation_solver_1d(const std::shared_ptr<mesh::mesh_1d<T>>& mesh,
                                                                                   const parameters_1d<T>& parameters,
                                                                                   const T time_step, const T initial_time)
    : _mesh{ mesh }, _right_part{ Eigen::Matrix<T, Eigen::Dynamic, 1>::Zero(mesh->nodes_count()) },
      _temperature_prev{ Eigen::Matrix<T, Eigen::Dynamic, 1>::Zero(mesh->nodes_count()) },
      _temperature_curr{ Eigen::Matrix<T, Eigen::Dynamic, 1>::Zero(mesh->nodes_count()) }, _parameters{ parameters },
      _time_step{ time_step }, _time{ initial_time }, _relaxation(mesh->segments_count()) {
    for (const size_t segment : mesh->segments())
        if (auto& phys = _parameters[segment].physical; phys.relaxation_time > T{ 0 }) {
            _relaxation[segment].container.resize(mesh->nodes_count(segment));
            _relaxation[segment].shift = mesh->nodes(segment).front();
...
template<class T, class I>
std::array<T, 2> nonstationary_heat_equation_solver_1d<T, I>::get_initial_values(
...
void nonstationary_heat_equation_solver_1d<T, I>::compute(const thermal_boundaries_conditions_1d<T>& boundaries_conditions) {
    ...
    reset_initial_values(_conductivity.inner, { settings.is_first_kind.front() ? T{ 1 } : _conductivity_initial_values.front(),
    ...
    for (const size_t parity : std::ranges::iota_view{ 0u, 2u }) {
        ...
        for (const size_t segment : mesh().segments()) {
            ...
            const T relaxation_factor =
                phys.relaxation_time > T{ 0 } && parity == segment % 2 ? std::exp(time() / phys.relaxation_time) : T{ 0 };
...
void nonstationary_heat_equation_solver_1d<T, I>::accumulate_relaxation_vector() {
    for (const size_t segment : mesh().segments())
        if (auto& phys = _parameters[segment].physical; phys.relaxation_time > T{ 0 }) {
            ...
            const auto& matrix = _relaxation_matrix[segment % 2].inner;
            for (const size_t row : mesh().nodes(segment)) {
                const size_t ind = matrix.outerIndexPtr()[row];
                _relaxation[segment][row] += value * matrix.valuePtr()[ind] * _temperature_prev[matrix.innerIndexPtr()[ind]];
                for (const size_t i : std::ranges::iota_view{ ind + 1, size_t(matrix.outerIndexPtr()[row + 1]) }) {
                    const T factor = value * matrix.valuePtr()[i];
                    _relaxation[segment][row] += factor * _temperature_prev[matrix.innerIndexPtr()[i]];
                    _relaxation[segment][matrix.innerIndexPtr()[i]] += factor * _temperature_prev[row];
                }
            }
        }
}
    ...
    for (const size_t segment : mesh().segments())
        if (auto& phys = _parameters[segment].physical; phys.relaxation_time > T{ 0 }) {
            ...
            for (const size_t node : mesh().nodes(segment))
                _right_part[node] -= value * _relaxation[segment][node];
        }
}
...
void nonstationary_heat_equation_solver_1d<T, I>::calc_step(const thermal_boundaries_conditions_1d<T>& boundaries_conditions,
    ...
    radiation_condition_1d<Is_Stationary>(_conductivity.inner, _right_part, boundaries_conditions, _temperature_prev,
                                          time_step());
