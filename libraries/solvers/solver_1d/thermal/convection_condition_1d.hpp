...
void convection_condition_1d(Eigen::SparseMatrix<T, Eigen::RowMajor, I>& K,
                             const stationary_thermal_boundary_condition_1d& boundary_condition,
                             const size_t index) {
    if (const auto* const condition = dynamic_cast<const stationary_convection_1d<T>*>(&boundary_condition))
