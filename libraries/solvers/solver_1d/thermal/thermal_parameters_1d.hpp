...
struct parameter_1d_base {
    ...
    T capacity = T{1};
    T density = T{1};
    T relaxation_time = T{1}; // added relaxation time
...
protected:
    explicit parameter_1d_base(const coefficients_t coefficients, const T capacity = T{1}, const T density = T{1}, const T relaxation_time = T{1}) noexcept // added relaxation time
        ...
        , capacity{capacity}
        , density{density} 
        , relaxation_time{relaxation_time} {} // added relaxation_time
...
public:
    ...
    constexpr parameter_1d() noexcept
        : parameter_1d_base<T>{Coefficients} {}     // added relaxation time (не уверен, что тут нужен)
