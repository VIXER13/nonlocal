...
class nonstationary_relax_time_heat_equation_solver_1d final {
    ...
    static std::array<T, 2> get_init_values(const Eigen::SparseMatrix<T, Eigen::RowMajor, I>& matrix) {
        /*  
            Метод, который возвращает первый и последний элемент на главной диагонали матрицы.
            RowMajor значит, что матрица хранится по строкам. Пример: https://eigen.tuxfamily.org/dox/group__TopicStorageOrders.html
        */
    ...
    static void reset_to_init_values(Eigen::SparseMatrix<T, Eigen::RowMajor, I>& matrix,
        ...
        /*
            Возвращает ссылки на значения первого и последнего элементов матрицы. ??
        */
...
void nonstationary_relax_time_heat_equation_solver_1d<T, I>::calc_step(const thermal_boundaries_conditions_1d<T>& boundaries_conditions,
    ...
    /*
        Насколько я понял здесь пересчитываются все знчения матриц и векторов по ходу времени.
        Здесь нужно добавить вектор P
        Вычленить время релаксации 
        Выполнить диагонализацию матрицы C cкорее всего НЕ тут, тк проводится всего один раз
    */        
