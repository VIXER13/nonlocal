...
class thermal_conductivity_assembler_1d final : public assembler_base_1d<T, I> {
  ...
  public:
    explicit thermal_conductivity_assembler_1d(finite_element_matrix_1d<T, I>& matrix,
...
template<class T, class I>
thermal_conductivity_assembler_1d<T, I>::thermal_conductivity_assembler_1d(
    finite_element_matrix_1d<T, I>& matrix, const std::shared_ptr<mesh::mesh_1d<T>>& mesh,
    const std::optional<utils::nodes_sequence>& nodes_to_assemble)
    : _base{ matrix, mesh, nodes_to_assemble } {}

template<class T, class I>
T thermal_conductivity_assembler_1d<T, I>::evaluate(const coefficient_t<T, 1>& conductivity, const size_t e,
...
template<class T, class I>
T thermal_conductivity_assembler_1d<T, I>::integrate_local(const coefficient_t<T, 1>& conductivity, const size_t e,
...
T thermal_conductivity_assembler_1d<T, I>::integrate_nonlocal(const coefficient_t<T, 1>& conductivity,
    ...
    for (const size_t qL : qnodes) {
        ...
        for (const size_t qNL : qnodes) {
            ...
            inner_integral +=
                influence(qcoordL, qcoordNL) * evaluate(conductivity, eNL, qNL) * el.weight(qNL) * el.qNxi(jNL, qNL);
        }
    ...
    _base::template calc_matrix(
        ...
        [this, &parameters](const size_t segment, const size_t eL, const size_t eNL, const size_t iL, const size_t jNL) {
            ...
            return nonlocal_weight(model.local_weight) *
                   integrate_nonlocal(physic.conductivity, model.influence, eL, eNL, iL, jNL);
        });
}
