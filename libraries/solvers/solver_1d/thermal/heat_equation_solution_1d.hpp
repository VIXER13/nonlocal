...
class heat_equation_solution_1d : public solution_1d<T> {
    ...
    std::vector<metamath::types::vector_with_shifted_index<T>> calc_nonlocal_flux(
        const std::vector<metamath::types::vector_with_shifted_index<T>>& local_flux);

  public:
    using _base::mesh;

    template<class Parameter, class Vector>
    explicit heat_equation_solution_1d(const std::shared_ptr<mesh::mesh_1d<T>>& mesh, const std::vector<Parameter>& parameters,
                                       const Vector& solution, const T time_step = T{ 0 });
...
heat_equation_solution_1d<T>::heat_equation_solution_1d(const std::shared_ptr<mesh::mesh_1d<T>>& mesh,
                                                        const std::vector<Parameter>& parameters, const Vector& solution,
                                                        const T time_step)
    : _base{ mesh, get_models(parameters) }, _temperature(solution.cbegin(), std::next(solution.cbegin(), mesh->nodes_count())),
      _parameters{ get_physical_parameters(parameters) }, _time_step{ time_step }, _relaxation(mesh->segments_count()) {
...
std::vector<metamath::types::vector_with_shifted_index<T>> heat_equation_solution_1d<T>::calc_local_flux() {
    ...
    const auto temperature_in_qnodes =
        is_any_nonlinear ? mesh::utils::from_nodes_to_qnodes(mesh(), temperature()) : std::vector<T>{};
    ...
    for (const size_t segment : mesh().segments()) {
        ...
        for (const size_t e : mesh().elements(segment)) {
            ...
            for (const size_t q : el.qnodes()) {
                flux[segment][qshift] -=
                    gradient[qshift] * evaluate(parameter(segment).conductivity, temperature_in_qnodes, e, q);
                ++qshift;
            }
        }
    }
...
template<class T>
std::vector<metamath::types::vector_with_shifted_index<T>> heat_equation_solution_1d<T>::calc_nonlocal_flux(
    const std::vector<metamath::types::vector_with_shifted_index<T>>& local_flux) {
    const auto& el = mesh().element();
