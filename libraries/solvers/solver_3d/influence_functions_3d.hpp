...
class constant_3d final {
  ...
  public:
    explicit constant_3d(const T& r) noexcept {
        set_radius(r);
    }
    explicit constant_3d(const std::array<T, 3>& r) noexcept {
        set_radius(r);
    }

    void set_radius(const T& r) noexcept {
        set_radius(std::array{ r, r });
    }
    ...
    const std::array<T, 3>& radius() const noexcept {
        return _r;
    }
...
class polynomial_3d final {
  ...
  public:
    explicit polynomial_3d(const T& r) noexcept {
        set_radius(r);
    }
    explicit polynomial_3d(const std::array<T, 2>& r) noexcept {
        set_radius(r);
    }
    ...
    void set_radius(const std::array<T, 2>& r) noexcept {
        _r = r;
        using metamath::function::power;
        _norm = 8 * r[0] * r[1] * r[2] * std::beta(T{ 3 } / P, Q + T{ 1 }) * power<3>(std::tgamma(T{ 1 } / N)) /
                (P * T{ N } * N * std::tgamma(T{ 3 } / N));
    }
...
class normal_distribution_3d final {
  ...
  public:
    explicit normal_distribution_3d(const T& r) noexcept {
        set_radius(r);
    }
    explicit normal_distribution_3d(const std::array<T, 3>& r) noexcept {
        set_radius(r);
    }

    void set_radius(const T& r) noexcept {
        set_radius(std::array{ r, r, r });
    }
    void set_radius(const std::array<T, 3>& r) noexcept {
        _r = r;
    ...
    const std::array<T, 2>& radius() const noexcept {
        return _r;
    }
    T norm() const noexcept {
        return _norm;
