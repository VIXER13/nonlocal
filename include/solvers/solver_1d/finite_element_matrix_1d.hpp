...
void finite_element_matrix_1d<T, I>::calc_shifts(const std::array<bool, 2> is_first_kind) {
    ...
    for(size_t node = 0; node < mesh().nodes_count(); ++node) {
        if (is_first_kind.front() && node == 0 || is_first_kind.back() && node == last_node)
            ...
            //std::cout << "e = " << e << " i = " << i << " neighbour = " << neighbour << " is_last_first_kind = " << is_last_first_kind << std::endl;
            //std::cout << "e = " << e << " segment = " << mesh().segment_number(e) << std::endl;
...
void finite_element_matrix_1d<T, I>::create_matrix_portrait(const std::array<bool, 2> is_first_kind) {
    ...
    //std::cout << "rows = " << matrix_inner().rows() << std::endl;
    //std::cout << "shifts" << std::endl;
    //for(const size_t row : std::ranges::iota_view{size_t{0}, size_t(matrix_inner().rows()) + 1})
    //    std::cout << matrix_inner().outerIndexPtr()[row] << ' ';
    //std::cout << std::endl;
