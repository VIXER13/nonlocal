...
void boundary_condition_first_kind_1d(Vector& f, const std::array<std::pair<boundary_condition_t, T>, 2>& boundary_condition,
    ...
    for(size_t b = 0; b < boundary_condition.size(); ++b)
        if (boundary_condition[b].first == boundary_condition_t::FIRST_KIND) {
            for(const auto& [i, val] : matrix_bound[b])
                f[i] -= val * boundary_condition[b].second;
            f[ind[b]] = boundary_condition[b].second;
        }
}

template<class T, class Vector>
void boundary_condition_second_kind_1d(Vector& f, const std::array<std::pair<boundary_condition_t, T>, 2>& boundary_condition,
                                       const std::array<size_t, 2>& ind) {
    for(size_t b = 0; b < boundary_condition.size(); ++b)
        if (boundary_condition[b].first == boundary_condition_t::SECOND_KIND ||
            boundary_condition[b].first == boundary_condition_t::THIRD_KIND)
            f[ind[b]] += boundary_condition[b].second;
}
