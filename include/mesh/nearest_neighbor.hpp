...
template<class T, class I>
void mesh_2d<T, I>::find_elements_neighbors(const T r) {
    const std::vector<std::array<T, 2>> centres = utils::get_centres_of_elements(*this);
...
// За соседа принимаем те элементы, центры которых попали в радиус.
template<class T, class I>
void mesh_2d<T, I>::find_nodes_neighbors(const T r) {
    const std::vector<std::array<T, 2>> centres = utils::get_centres_of_elements(*this);
