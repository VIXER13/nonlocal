...
heat_equation_solver<T, I>::stationary(const std::vector<boundary_condition<T>>& bounds_cond, const Right_Part& right_part, 
    ...
    std::cout << "nonzero K after: " << K.nonZeros() << std::endl;
    std::cout << "nonzero K_bound after: " << K_bound.nonZeros() << std::endl;

    // Eigen::MatrixXd mat = Eigen::MatrixXd{K}.transpose().block(0, 0, K.rows(), K.rows());
    // for(int i = 0; i < mat.rows(); ++i)
    //     for(int j = 0; j < mat.cols(); ++j)
    //         if (std::abs(mat(i, j)) < 1e-5)
    //             mat(i, j) = 0;
    // std::cout << std::endl << std::endl << mat << std::endl << std::endl;

    // mat = Eigen::MatrixXd{K_bound}.transpose().block(0, 0, K_bound.rows(), K_bound.rows());
    // for(int i = 0; i < mat.rows(); ++i)
    //     for(int j = 0; j < mat.cols(); ++j)
    //         if (std::abs(mat(i, j)) < 1e-5)
    //             mat(i, j) = 0;
    ...
    std::cout << std::endl << std::endl << Eigen::MatrixXd{K_bound} << std::endl << std::endl;
    ...
    Eigen::ConjugateGradient<Eigen::SparseMatrix<T, Eigen::ColMajor, I>, Eigen::Lower> solver;
    solver.compute(K);
