...
protected:
    ...
    T _p = -1. / 3.; // Значение по умолчанию даёт нам классический вариант квадратичных серендиповых элементов.
    ...
    //                                                        |       |
    //                                                        0---1---2
    static constexpr std::array<std::array<T, 2>, 8> nodes = { -1., -1.,
                                                                0., -1.,
                                                                1., -1.,
                                                                1.,  0.,
                                                                1.,  1.,
    ...
    static constexpr auto basis = std::make_tuple(
        ...
        -(1.-xi*xi)   * (1.-eta) * ((0.5625*p-0.3125)             + (0.5625*p+0.1875)*eta),
         (1.+xi)      * (1.-eta) * ((0.5625*p-0.0625)*(1.-xi+eta) - (0.5625*p+0.1875)*xi*eta),
