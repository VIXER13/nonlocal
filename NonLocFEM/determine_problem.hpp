...
class _determine_problem final {
    ...
    template<std::floating_point T>
    friend void mechanical_nonstationary_2d(std::shared_ptr<mesh::mesh_2d<T>>& mesh, const nlohmann::json& config,
                                            const config::save_data& save);
    ...
    template<std::floating_point T>
    friend std::optional<solver_2d::mechanical::mechanical_solution_2d<T>> mechanical_2d(std::shared_ptr<mesh::mesh_2d<T>>& mesh,
                                                                                         const nlohmann::json& config,
                                                                                         const config::problem_t problem,
...
void problems_1d(const nlohmann::json& config, const config::save_data& save, const config::task_data& task) {
    ...
    if (parallel::MPI_rank() != 0) {
        logger::warning() << "Calculations are available only on the master process. "
                             "The current process has completed its work."
                          << std::endl;
        return;
    }
...
template<std::floating_point T>
std::optional<solver_2d::thermal::heat_equation_solution_2d<T>> thermal_stationary_2d(std::shared_ptr<mesh::mesh_2d<T>>& mesh,
                                                                                      const nlohmann::json& config,
                                                                                      const config::problem_t problem) {
    ...
    return solver_2d::thermal::stationary_heat_equation_solver_2d(
        mesh, config::read_thermal_parameters_2d<T>(config["materials"], "materials"),
...
void thermal_nonstationary_2d(std::shared_ptr<mesh::mesh_2d<T>>& mesh, const nlohmann::json& config,
    ...
    const auto auxiliary =
        config::read_stationary_equation_parameters_2d<T>(config.value("auxiliary", nlohmann::json::object()), "auxiliary");
    solver.compute(config::read_thermal_parameters_2d<T>(config["materials"], "materials"),
                   config::read_thermal_boundaries_conditions_2d<T>(config["boundaries"], "boundaries"), time.time_step,
                   auxiliary.right_part, auxiliary.initial_distribution, time.initial_time);
    {
...
template<std::floating_point T>
void mechanical_nonstationary_2d(std::shared_ptr<mesh::mesh_2d<T>>& mesh, const nlohmann::json& config,
                                 const config::save_data& save) {
    using DP = _determine_problem;
...
template<std::floating_point T>
std::optional<solver_2d::mechanical::mechanical_solution_2d<T>> mechanical_2d(std::shared_ptr<mesh::mesh_2d<T>>& mesh,
                                                                              const nlohmann::json& config,
                                                                              const config::problem_t problem,
                                                                              const std::vector<T>& delta_temperature) {
    using DP = _determine_problem;
    ...
    return solver_2d::mechanical::equilibrium_equation(
        mesh, config::read_mechanical_parameters_2d<T>(config["materials"], "materials"),
        config::read_mechanical_boundaries_conditions_2d<T>(config[boundaries_field], boundaries_field), delta_temperature);
}
...
void problems_2d(const nlohmann::json& config, const config::save_data& save, const config::task_data& task) {
    ...
    case config::analysis_type_t::Stationary: {
        const std::optional<solver_2d::thermal::heat_equation_solution_2d<T>> thermal_solution =
            thermal_stationary_2d<T>(mesh, config, task.problem);
...
void determine_problem(const nlohmann::json& config) {
    ...
    if (const config::task_data task{ config["task"], "task" }; task.dimension == 1)
        ...
        throw std::domain_error{ "Problem dimension " + std::to_string(task.dimension) + " is not supported" };
}
