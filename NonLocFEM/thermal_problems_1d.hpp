...
void save_solution(const solver_1d::thermal::heat_equation_solution_1d<T>& solution, const config::save_data& save,
...
void solve_thermal_1d_problem(const nlohmann::json& config, const config::save_data& save,
    ...
    const auto auxiliary =
        config::thermal_auxiliary_data_1d<T>{ config.value("auxiliary", nlohmann::json::object()), "auxiliary" };
    ...
    default:
        throw std::domain_error{ "Unknown analysis type for thermal 1d problem." };
    }
}
