...
void save_solution(const solver_1d::mechanical::mechanical_equation_solution_1d<T>& solution, const config::save_data& save,
                   const std::optional<uint64_t> step = std::nullopt) {
    if (step.has_value())
        logger::info() << "save step " << *step << std::endl;
    const std::filesystem::path path = step
                                           ? save.make_path(save.get_name("csv", "solution") + "_" + std::to_string(*step), "csv")
                                           : save.path("csv", "csv", "solution");
    mesh::utils::save_as_csv(path, solution.mesh(),
                             { { "displacement", solution.displacement() }, { "stress", solution.stress() } }, save.precision());
}

template<std::floating_point T, std::integral I>
void solve_mechanical_1d_problem(const nlohmann::json& config, const config::save_data& save,
                                 const config::analysis_type_t analysis_type) {
    ...
    const auto initial_distribution_input = [initial_distribution_input = auxiliary.initial_distribution](T x) {
        return std::visit(metamath::types::visitor{ [](const T value) { return value; },
                                                    [&x](const spatial_dependency<T, 1>& value) { return value(x); },
                                                    [](const auto&) {
                                                        throw std::domain_error{ "Unsuported initial distribution format." };
                                                        return T{ 0 };
                                                    } },
    ...
    case config::analysis_type_t::Stationary: {
        auto solution = stationary_mechanical_equation_solver_1d<T, I>(
            ...
            stationary_equation_parameters_1d<T>{ .right_part = right_part_input,
    ...
    case config::analysis_type_t::Time_Harmonic: {
        ...
        for (size_t i = 0; i < sweep.frequencies.size(); ++i) {
            ...
            auto solution = harmonic_mechanical_equation_solver_1d<T, I>(
                mesh, parameters, boundaries_conditions,
                time_harmonic_equation_parameters_1d<T>{ .right_part = right_part_input,
                                                         .initial_distribution = initial_distribution_input,
            ...
            logger::info() << " freq #" << i << ": " << sweep.frequencies[i]
                           << " | time = " << std::chrono::duration_cast<std::chrono::seconds>(finish - start).count() << " sec."
                           << std::endl;
        }
        break;
    }
