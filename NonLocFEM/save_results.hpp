...
void save_csv(const std::optional<solver_2d::thermal::heat_equation_solution_2d<T>>& thermal_solution,
    ...
    if (mechanical_solution) {
        data.emplace_back(
            std::pair{ std::array{ "displacement_x"s, "displacement_y"s }, std::cref(mechanical_solution->displacement()) });
        if (mechanical_solution->is_strain_and_stress_calculated()) {
            data.emplace_back(
                std::pair{ std::array{ "strain_11"s, "strain_22"s, "strain_12"s }, std::cref(mechanical_solution->strain()) });
            data.emplace_back(
                std::pair{ std::array{ "stress_11"s, "stress_22"s, "stress_12"s }, std::cref(mechanical_solution->stress()) });
        }
    }
    if (data.empty())
    ...
    const std::filesystem::path path =
        step ? save.make_path(std::to_string(*step) + "_" + save.get_name("csv"), "csv") : save.path("csv", "csv");
...
void save_vtk(const std::optional<solver_2d::thermal::heat_equation_solution_2d<T>>& thermal_solution,
    ...
    if (!thermal_solution && !mechanical_solution)
        throw std::logic_error{ "Nothig to save." };
    const std::filesystem::path path =
        step ? save.make_path(std::to_string(*step) + "_" + save.get_name("vtk"), "vtk") : save.path("vtk", "vtk", "solution");
