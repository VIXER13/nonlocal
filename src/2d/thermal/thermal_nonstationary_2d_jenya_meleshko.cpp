...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (p1 < 0.999) {
			...
			mesh_proxy->find_neighbours(std::max(r[0], r[1]) + 0.015, nonlocal::mesh::balancing_t::MEMORY); // 0.05 это некая гарантированная добавка// 0.1 — - 0.15 ~0.141 0.5 --- 0.70// 0.2 ---0.28 !!!!!!!!!
            ...
            boundary_conditions = {
				{   "Down",																	//
					{   nonlocal::thermal::boundary_condition_t::FLUX,						//
						[](const double t, const std::array<double, 2>& x) { return 100; },	//return (-0.1 < x[0] & 0.1 > x[0] ? 500 : 0);
					}																		//
				},																			//
                {   "Right",
                    {   nonlocal::thermal::boundary_condition_t::FLUX,
